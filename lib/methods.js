Meteor.methods({
  ...
  makeQueueMove: function(queueId, move, playerId) {
    ...
    if (chess.game_over()) {
      ...
      if (result === 'draw') {
          ...
          Queues.update({_id: queueId}, {$set: {
            activePlayers: activePlayersArray
          }});
          Queues.update({_id: queueId}, {$set: {
            moves: '', board: (new Chess).fen(), result: null
          }});
          Queues.update({_id: queueId}, {$set: {queue: queueArray}},
            function(err) {
              if (err) throw err;
        ...
        Queues.update({_id: queueId}, {$set: {queue: queueArray}},
          function(err) {
            ...
            var message = getUsername(winner) + ' won against ' + getUsername(loser) + ' by checkmate. ' + getUsername(activePlayersArray[1]) + ' is now an active player!';
  ...
  playerForfeit: function(queueId, playerId) {
    ...
    Queues.update({_id: queueId}, {$set: {queue: queueArray}},
      function(err) {
        ...
        var message = getUsername(winner) + ' won against ' + getUsername(loser) + ' by forfeit. ' + getUsername(activePlayersArray[1]) + ' is now an active player!';
